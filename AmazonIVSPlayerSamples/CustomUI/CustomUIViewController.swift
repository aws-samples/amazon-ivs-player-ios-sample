...
class CustomUIViewController: UIViewController, CustomUISourceViewDelegate {
    ...
    let sourcesSaveKey: String = "customui_sources_history_data"
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        if player?.path == nil {
            loadStream(sources.first!.urlString)
        }
    }
    ...
    func loadStream(_ urlString: String) {
        if let url = URL(string: urlString) {
            player?.load(url)
            playTapped(self)
            if sources.filter({ $0.urlString == urlString }).count == 0 {
                sources.append(Source(urlString, urlString))
                saveData()
            }
        }
    }
    ...
    func loadData() {
        if let data = UserDefaults.standard.value(forKey: sourcesSaveKey) as? Data, let savedEntities = try? PropertyListDecoder().decode([Source].self, from: data) {
            ...
            sources = [
                Source("Live stream Landscape", "https://fcc3ddae59ed.us-west-2.playback.live-video.net/api/video/v1/us-west-2.893648527354.channel.DmumNckWFTqz.m3u8"),
                Source("Recorded video Landscape", "https://d6hwdeiig07o4.cloudfront.net/ivs/956482054022/cTo5UpKS07do/2020-07-13T22-54-42.188Z/OgRXMLtq8M11/media/hls/master.m3u8")
            ]
        }
    }
...
extension CustomUIViewController: IVSPlayer.Delegate {
